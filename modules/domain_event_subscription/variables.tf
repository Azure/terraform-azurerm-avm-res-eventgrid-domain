...
The resource ID of the Event Grid Domain to create the subscription on.
This can be an existing domain not managed by the parent module.
DESCRIPTION

  validation {
    condition     = can(regex("^/subscriptions/[^/]+/resourceGroups/[^/]+/providers/Microsoft.EventGrid/domains/[^/]+$", var.event_grid_domain_resource_id))
    error_message = "event_grid_domain_resource_id must be a valid Event Grid Domain resource ID."
  }
}
